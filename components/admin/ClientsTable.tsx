  ...
  return (
    <>
      <input
        ...
        className="w-full mb-6 bg-zinc-900/50 border border-zinc-800 rounded-xl px-4 py-3 placeholder:text-current placeholder:opacity-50 focus:outline-none focus:ring-2 focus:ring-secondary/50 focus:border-secondary transition-all"
      />

      <div className="overflow-x-auto -mx-6 px-6">
        <table className="w-full text-left border-collapse min-w-[600px]">
          <thead>
            <tr className="border-b border-zinc-800 opacity-60 text-sm">
            ...
            {rows.length === 0 ? (
              ...
              rows.map((client) => (
                <tr key={client.id} className="hover:bg-zinc-900/30 transition-colors">
                  <td className="py-4 pl-4 font-medium">{client.name}</td>
                  <td className="py-4 opacity-70">{client.phone || '-'}</td>
                  <td className="py-4">
                    <span className="font-medium">
                      {client.current_visits}
                    </span>
                  </td>
                  <td className="py-4 opacity-60 text-sm" suppressHydrationWarning>
